...
- **Security posture:** `contextIsolation` ON and `nodeIntegration` OFF unconditionally, and no `@electron/remote`. The renderer `sandbox` is ON for every window unconditionally, dev/unpackaged runs included (`browserWindowConfig.ts`); only `webSecurity` is relaxed in dev (`webSecurity: isPackaged`), which lifts same-origin/CORS checks so the Vite-served renderer can call local services and load `file://` assets. Every window also gets one window-open handler (`externalizeWindowOpen`, registered in `web-contents-created`) that denies in-app windows and hands `https:` to the OS browser. IPC handlers that reach privileged side effects validate the sender frame via `isPermittedFrameUrl` (`src/main/ipc/frameValidation.ts`).
...
- **Conditional Cloudflare R2 custody for new SaaS attachments** ([ADR-0038](adr/0038-attachment-storage-placement-and-delivery.md)). Production currently writes new attachments to `legacy`; R2 custody begins only after the explicit operator cutover. The selector, per-object routing, and version-aware client/server plumbing are deployed, but the cutover has not occurred. New attachments are encrypted client-side before upload, so their contents stay unreadable to the storage vendor; R2 will store the encrypted attachment bytes and expose only object count, size, upload timing, and a bucket key whose server-identifying segment is a keyed one-way transform (`HMAC-SHA256` over a secret pepper, truncated) — it hides the underlying server or conversation identifier from the vendor while still preserving grouping, because the transform is deterministic. Downloads stay proxied through the control plane, never a direct vendor URL. This does not newly expose fetches to observation: Cloudflare already observes client IP, `file_id` in cleartext, size, and timing on every attachment download **today**, because `api.concordvoice.chat` is Cloudflare-proxied — the change hides content from the storage vendor, not from Cloudflare.
...
- **Vendor write-default flip for attachments** ([ADR-0038](adr/0038-attachment-storage-placement-and-delivery.md)) — the *mechanism* shipped; the *cutover* has not. Per-object placement is live: `media_files.storage_backend` exists (migrations `000114`–`000115`), `reap_attempts` lands in `000116`, `internal/media/backend_store.go` resolves the store per row on the read, delete, and write rails, and the boot-time backend registry is wired. The selector stays `legacy` through a zero-downtime split: PR2A deploys 000116 alone, confirms schema version 116, then creates `idx_media_files_unreaped_retry_rank` concurrently on `((COALESCE(storage_backend, 'legacy')), reap_attempts, deleted_at)` with the existing partial predicate plus `reap_attempts > 0`, verifies it valid, and retains the existing fresh-row index. PR2B then deploys the corrected backend-fair reaper and flat selector-blind required R2 provisioning. The protected registry-backed production R2 probe passed as run 33350980852 on 2026-08-30; a fresh probe remains a final-cutover gate. Retry scheduling partitions by normalized backend, reserves up to 10% of each stride for retries fairly across backend groups ordered by backend rank then `(reap_attempts, deleted_at)`, fills the remainder with fresh rows, and uses retry rows to backfill unused capacity; eligible distinct normalized backend groups must stay below 100. Before cutover, query and monitor that count and stop/investigate before it reaches 100. After deployment and a 2h30m session drain, an authorized operator may flip the selector. R2 credentials remain required on rollback. Profile media (`avatars/`, `banners/`, `server-icons/`, `server-banners/`, `dm-icons/`) is MinIO-resident **permanently** regardless, because the server decodes and re-encodes it and therefore holds it as plaintext wherever it lands. MinIO remains the only backend for self-hosted, dev, and the #210 air-gapped bundle, and delivery stays proxied through the control plane either way — this ADR does not ship presigned-direct.
