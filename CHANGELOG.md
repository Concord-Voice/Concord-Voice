...
- **Build and deployment tools no longer carry the four affected Undici resolutions** ([#3305](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3305), [#3133](https://github.com/Concord-Voice/Concord-Voice-Alpha/issues/3133)) — desktop and Admin now resolve patched Undici releases across their existing major lines, and the Apple SSO and invite Workers contain Wrangler's unreleased Miniflare dependency until Wrangler ships that fix. Worker deployments now use the audited lockfile's Wrangler instead of fetching a second version at deploy time. Concord Voice runtime code did not use the affected APIs.
...
- **A broken update can no longer look like a successful one** ([#3106](https://github.com/Concord-Voice/Concord-Voice-Alpha/issues/3106)) — the check that decided whether an update had worked only asked whether the service had started, not whether it could actually reach the database it needs. A server that came back up but could not reach its database answered "healthy" while rejecting every signed-in request, and the update was recorded as a success. That check now asks whether the service can really serve, so an update that lands in that state fails and is caught instead of going live.

- **Closing Picture-in-Picture voice during startup now disconnects it cleanly** ([#3128](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3128)) — closing the PiP window before voice setup finishes now cancels its pending authenticated session instead of registering it after the window is gone.
...
- **Production now validates the R2 attachment destination before cutover** ([#3050](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3050)) — production refuses to start without the configured R2 credentials even while legacy storage remains selected, retries purge work fairly across backends, and bounds object-delete timeouts. This prepares operations for cutover without moving writes yet.
- **Desktop production dependencies were refreshed** ([#3069](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3069), [#3062](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3062), [#3078](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3078)) — `jose` and `lucide-react` moved to their reviewed releases, and the build-only `xmldom` lockfile resolution moved past a security advisory.
- **Admin, desktop, and media-plane development tooling was refreshed** ([#3068](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3068), [#3063](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3063), [#3064](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3064), [#3066](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3066), [#3067](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3067), [#3059](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3059), [#3056](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3056), [#3060](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3060)) — linting, build, test, and type packages advanced without changing runtime behavior.
- **Browser compatibility metadata no longer resolves to advisory-affected releases** ([#3080](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3080)) — the admin and media-plane lockfiles now use the reviewed metadata chain.
- **Weekly dependency triage now fails closed on missing PR CI and recognizes canonical advisory IDs** ([#3081](https://github.com/Concord-Voice/Concord-Voice-Alpha/pull/3081)) — type-only React update groups remain on the fast path.
