...
describe('SSOEagerUnlock', () => {
  it('renders prompt and unlocks on correct passphrase', async () => {
    ...
    // through — the 6th arg (sinceEpoch) is load-bearing for the P1-B fence, so
  ...
  it('does NOT admit when the token was invalidated WITHOUT an E2EE teardown (Codex P1, PR #2337)', async () => {
    // handleRefreshFailure's `phase !== 'stable'` path clears the access token
    // without clearKeys(), so the teardown epoch never advances and the epoch
    // check passes. The token-lifecycle admit gate must catch the ownership
    // loss before onUnlock().
  ...
  it('still admits when the keys fetch transparently refreshed the session (Codex P1, PR #2337)', async () => {
    ...
    const { useAuthStore } = await import('@/renderer/stores/authStore');
    useAuthStore.getState().setAccessToken('sso-token-A'); // pre-refresh bootstrap identity
    mockE2eeInitialize.mockResolvedValueOnce(undefined);
    mockFetch.mockImplementationOnce(async () => {
      // The transparent refresh lands during the keys fetch.
      useAuthStore.getState().setAccessToken('sso-token-B');
      useAuthStore.getState().setSessionId('sso-session-1');
      return {
        ok: true,
        status: 200,
        headers: new Headers({ 'Content-Type': 'application/json' }),
