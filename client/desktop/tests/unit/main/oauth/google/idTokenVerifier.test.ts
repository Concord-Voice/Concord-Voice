...
import {
  verifyGoogleIDToken,
  GOOGLE_ISSUERS,
} from '../../../../../src/main/oauth/google/idTokenVerifier';
...
const expectCode = async (p: Promise<unknown>) =>
  p.then(
    () => null,
    (e) => (e instanceof GoogleFlowError ? e.code : `unexpected:${e}`)
  );
...
describe('verifyGoogleIDToken', () => {
  ...
  it('rejects wrong audience', async () =>
    ...
    ).toBe('google_id_token_invalid'));
  it('rejects nonce mismatch', async () =>
    expect(
      await expectCode(
        verifyGoogleIDToken({
          idToken: await mint({ nonce: 'x' }),
          clientId: CLIENT_ID,
          expectedNonce: NONCE,
          jwks,
        })
  ...
  it('rejects HS256 algorithm confusion', async () => {
    const hs = await new jose.SignJWT({ nonce: NONCE, sub: 's' })
      ...
      .setAudience(CLIENT_ID)
      .setExpirationTime('5m')
      .sign(new TextEncoder().encode('shared'));
    expect(
      await expectCode(
        verifyGoogleIDToken({ idToken: hs, clientId: CLIENT_ID, expectedNonce: NONCE, jwks })
      )
    ).toBe('google_id_token_invalid');
  });
  it('rejects expired token', async () => {
    ...
    expect(
      await expectCode(
        verifyGoogleIDToken({ idToken: expired, clientId: CLIENT_ID, expectedNonce: NONCE, jwks })
      )
    ).toBe('google_id_token_invalid');
  });
  it('rejects missing sub', async () => {
    const t = await new jose.SignJWT({ nonce: NONCE })
      .setProtectedHeader({ alg: 'RS256', kid: 'k1' })
      .setIssuer('https://accounts.google.com')
      .setAudience(CLIENT_ID)
      .setExpirationTime('5m')
    ...
    expect(
      await expectCode(
        verifyGoogleIDToken({ idToken: t, clientId: CLIENT_ID, expectedNonce: NONCE, jwks })
      )
    ).toBe('google_id_token_invalid');
  });
});
