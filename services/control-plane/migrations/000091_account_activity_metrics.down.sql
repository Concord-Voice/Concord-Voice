-- Migration: account_activity_metrics (down)
-- Purpose: Remove account/activity metric rows before restoring the original
-- 52-key catalog, then remove the private activity marker.
-- This deliberate feature rollback discards derived telemetry and must run with
-- the matching producer quiesced. The table lock closes the delete/constraint
-- race if an in-flight collector transaction remains.

LOCK TABLE ops_metric_samples, ops_metric_rollups IN ACCESS EXCLUSIVE MODE;

DELETE FROM ops_metric_rollups
WHERE metric_key IN (
...
DROP INDEX IF EXISTS idx_users_ops_last_active_at;

ALTER TABLE users
    DROP COLUMN IF EXISTS ops_last_active_at;
