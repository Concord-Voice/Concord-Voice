-- Migration: camera_layering_ops_metrics (down)
-- Purpose: Narrow the closed metric catalog back to its 62-key 000113 form.
--
-- Rows carrying the retired keys must be removed BEFORE the constraint is
...
-- 8d rollups), so this touches at most a few days of two scalar series --
-- aggregate counts, never identity data.

-- ACCESS EXCLUSIVE before the removals, matching the 000091 and 000113
-- precedent. Without it a collector still running the pre-rollback binary can
-- INSERT a row carrying a retired key between the removal and the constraint
-- re-add, and the ALTER then fails on data this migration believed it had
-- removed.
LOCK TABLE ops_metric_samples, ops_metric_rollups IN ACCESS EXCLUSIVE MODE;

DELETE FROM ops_metric_samples WHERE metric_key IN ('media_camera_layering_gate_flips_total', 'media_camera_pressure_demands_total');
DELETE FROM ops_metric_rollups WHERE metric_key IN ('media_camera_layering_gate_flips_total', 'media_camera_pressure_demands_total');
