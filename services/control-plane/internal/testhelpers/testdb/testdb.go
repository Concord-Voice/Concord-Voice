// Package testdb provides cycle-free PostgreSQL fixtures for integration tests.
// It deliberately does not import the API router or websocket packages.
package testdb
...
import (
	...
	_ "github.com/lib/pq" // Register the PostgreSQL database/sql driver used by the fixture.
)
...
// DatabaseURL returns the configured database URL or the shared test default.
...
// CreateUser inserts the smallest user row required by database-focused tests.
func CreateUser(t *testing.T, db *sql.DB) uuid.UUID {
	...
	if err != nil {
		t.Fatalf("testdb: failed to create user: %v", err)
	}
	return id
}
...
func truncateAllTablesOnce(db *sql.DB) error {
	_, err := db.Exec(`TRUNCATE
		...
		dm_block_reconciliations,
		dm_block_voice_ejections,
		credential_epoch_voice_ejections,
...
func migrationsPath() string {
	...
	// testdb is at internal/testhelpers/testdb/testdb.go
