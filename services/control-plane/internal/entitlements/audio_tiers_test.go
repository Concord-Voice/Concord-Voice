...
func TestServerAudioCeilingTier(t *testing.T) {
	...
	assert.Equal(t, "studio", entitlements.ServerAudioCeilingTier(entitlements.TierMach1))
...
func TestAudioTierAllowedForServer(t *testing.T) {
	...
	// Mach: studio allowed.
	assert.True(t, entitlements.AudioTierAllowedForServer("studio", entitlements.TierMach1))
	// Unknown tier fails closed.
	assert.False(t, entitlements.AudioTierAllowedForServer("garbage", entitlements.TierGroundspeed))
	assert.False(t, entitlements.AudioTierAllowedForServer("", entitlements.TierMach1))
}
...
func TestMediaForChannel_FreeUser_StudioChannel_OnMachServer_Uplifted(t *testing.T) {
	...
	got := entitlements.MediaForChannel(entitlements.TierFree, entitlements.TierMach1, "studio")
...
func TestMediaForChannel_UnknownChannelTier_FallsBackToPersonal(t *testing.T) {
	assert.Equal(t,
		...
		entitlements.MediaForChannel(entitlements.TierFree, entitlements.TierMach1, "garbage"))
}

// TestServerAudioCeilingTier_Ladder locks the ADR-0028 semantic: audio stays
// BINARY over the ladder — groundspeed → standard, every Mach level and
// selfhost → studio, unknown/retired ("mach") → standard (fail closed).
func TestServerAudioCeilingTier_Ladder(t *testing.T) {
	cases := map[string]string{
		entitlements.TierGroundspeed: "standard",
		entitlements.TierMach1:       "studio",
		entitlements.TierMach2:       "studio",
		entitlements.TierMach3:       "studio",
		entitlements.TierSelfHost:    "studio",
		"mach":                       "standard", // retired pre-ladder string
		"":                           "standard",
	}
	for tier, want := range cases {
		t.Run("tier="+tier, func(t *testing.T) {
			assert.Equal(t, want, entitlements.ServerAudioCeilingTier(tier))
		})
	}
}
