...
func TestResolveTargetKeyVersionDM(t *testing.T) {
	...
	t.Run("explicit version wins without touching the database", func(t *testing.T) {
		explicit := 7
		// A nil querier proves no query is issued on this path.
		v, err := resolveTargetKeyVersionDM(context.Background(), nil, uuid.NewString(), &explicit)
		require.NoError(t, err)
		assert.Equal(t, 7, v)
	})

	t.Run("unknown conversation defaults to version 1", func(t *testing.T) {
		v, err := resolveTargetKeyVersionDM(context.Background(), db, uuid.NewString(), nil)
		require.NoError(t, err)
	...
	t.Run("a read error fails closed instead of clamping to 1", func(t *testing.T) {
		...
		v, err := resolveTargetKeyVersionDM(context.Background(), broken, uuid.NewString(), nil)
		require.Error(t, err,
