...
func TestCacheGetMiss(t *testing.T) {
	...
	perm, ok, _ := cache.Get(ctx, "server1", "user1", "")
...
func TestCacheSetAndGetServerLevel(t *testing.T) {
	...
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "", expected)
...
func TestCacheSetAndGetChannelLevel(t *testing.T) {
	...
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "channel1", expected)

	perm, ok, _ := cache.Get(ctx, "server1", "user1", "channel1")
	assert.True(t, ok, "cache hit should return true")
	assert.Equal(t, expected, perm)
}
...
func TestCacheServerAndChannelKeysAreSeparate(t *testing.T) {
	...
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "", serverPerm)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "channel1", channelPerm)

	// They should be independent
	perm1, ok, _ := cache.Get(ctx, "server1", "user1", "")
	assert.True(t, ok)
	assert.Equal(t, serverPerm, perm1)

	perm2, ok, _ := cache.Get(ctx, "server1", "user1", "channel1")
	assert.True(t, ok)
...
func TestCacheInvalidateUser(t *testing.T) {
	...
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "", rbac.PermViewTextChannels)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "channel1", rbac.PermSendMessages)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "channel2", rbac.PermKick)
	...
	_, ok, _ := cache.Get(ctx, "server1", "user1", "")
	assert.False(t, ok, "server-level cache should be invalidated")

	_, ok, _ = cache.Get(ctx, "server1", "user1", "channel1")
	assert.False(t, ok, "channel1 cache should be invalidated")

	_, ok, _ = cache.Get(ctx, "server1", "user1", "channel2")
...
func TestCacheInvalidateDoesNotAffectOtherUsers(t *testing.T) {
	...
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "", rbac.PermViewTextChannels)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user2", "", rbac.PermSendMessages)
	...
	_, ok, _ = cache.Get(ctx, "server1", "user2", "")
...
func TestCacheInvalidateServer(t *testing.T) {
	...
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "", rbac.PermViewTextChannels)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user2", "", rbac.PermSendMessages)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "channel1", rbac.PermKick)

	// Also set for a different server (should not be affected)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server2", "user1", "", rbac.PermBan)
	...
	_, ok, _ = cache.Get(ctx, "server2", "user1", "")
...
func TestCacheInvalidateChannel(t *testing.T) {
	...
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "channel1", rbac.PermViewTextChannels)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user2", "channel1", rbac.PermSendMessages)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "channel2", rbac.PermKick)
	testhelpers.PublishPermissionCache(t, ts.Redis, "server1", "user1", "", rbac.PermBan)
	...
	_, ok, _ = cache.Get(ctx, "server1", "user2", "channel1")
...
func TestCacheGetDifferentChannelReturnsOwnValue(t *testing.T) {
	...
	_, ok, _ := cache.Get(ctx, "server1", "user1", "channel999")
