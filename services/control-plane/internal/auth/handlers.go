...
// SetKeyRevocationBroadcaster wires committed theft rotations to WebSocket clients.
func (h *Handler) SetKeyRevocationBroadcaster(broadcaster keyrotation.Broadcaster) {
	h.keyBroadcaster = broadcaster
}

// SetInitialDistributorChecker wires fresh channel-VIEW checks into theft-triggered rotations.
func (h *Handler) SetInitialDistributorChecker(checker keyrotation.InitialDistributorChecker) {
	h.initialDistributorChecker = checker
}
...
func decodeE2EEKeys(publicKeyB64, wrappedKeyB64, saltB64 string) (publicKey, wrappedKey, salt []byte, err error) {
	...
	if err := ValidateE2EEPublicKey(publicKey); err != nil {
...
// ValidateE2EEPublicKey accepts only the RSA-4096 SPKI public keys generated by clients.
func ValidateE2EEPublicKey(publicKey []byte) error {
	parsed, err := x509.ParsePKIXPublicKey(publicKey)
	if err != nil {
		return fmt.Errorf("Invalid public key format") //nolint:staticcheck // ST1005: user-facing message rendered directly in UI
	}
...
func (h *Handler) triggerTheftKeyRevocations(userID string) {
	...
	defer func() {
		if closeErr := rows.Close(); closeErr != nil {
			h.log.Error("Failed to close channels for theft key revocation", "error", closeErr, "user_id", userID)
		}
	}()
	...
	if err := rows.Err(); err != nil {
		h.log.Error("Failed to list channels for theft key revocation", "error", err, "user_id", userID)
		return
	}
	if err := rows.Close(); err != nil {
		h.log.Error("Failed to close channels for theft key revocation", "error", err, "user_id", userID)
		return
	}

	for _, channelID := range channelIDs {
		rotation, err := h.recordTheftKeyRevocation(userID, channelID)
		if err != nil {
			continue
		}
		if rotation != nil && h.keyBroadcaster != nil {
			h.keyBroadcaster(*rotation)
		}
	}
...
func (h *Handler) recordTheftKeyRevocation(userID, channelID string) (*keyrotation.Rotation, error) {
	tx, err := h.db.BeginTx(context.Background(), nil)
	if err != nil {
		h.log.Error("Failed to begin theft key revocation", "error", err, "channel_id", channelID)
		return nil, err
	}
	rotation, err := keyrotation.RecordKeyRevocationTx(
		context.Background(), tx, h.initialDistributorChecker,
		channelID, "theft_detected", userID, userID,
	)
	if err == nil {
		err = tx.Commit()
	}
	if err != nil {
		if rbErr := tx.Rollback(); rbErr != nil && !errors.Is(rbErr, sql.ErrTxDone) {
			h.log.Error("Failed to rollback theft key revocation", "error", rbErr, "channel_id", channelID)
		}
		h.log.Error("Failed to record key revocation for theft", "error", err, "channel_id", channelID)
		return nil, err
	}
	return rotation, nil
}
...
func (h *Handler) RecoveryResetAccount(c *gin.Context) {
	...
	ops = append(ops,
		...
		recoveryTxOp{`DELETE FROM channel_initial_key_distributions WHERE creator_id = $1`,
			[]interface{}{claims.UserID}, "Failed to clear incomplete channel distributions"},
